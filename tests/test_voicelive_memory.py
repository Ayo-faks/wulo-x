...
class TestBackgroundTaskTracking:
    ...
    def _create_task_tracker(self):
        """Create a handler shell that exercises the production task methods."""
        from apps.artagent.backend.voice.voicelive.handler import VoiceLiveSDKHandler

        tracker = object.__new__(VoiceLiveSDKHandler)
        tracker._pending_background_tasks = set()
        return tracker
    ...
    @pytest.mark.asyncio
    async def test_background_task_error_logging(self, caplog):
        ...
        try:
            ...
            with caplog.at_level(
                logging.DEBUG,
                logger="voicelive.handler",
            ):
                task = tracker._background_task(failing_coro(), label="failing")
                task.add_done_callback(lambda _task: callback_complete.set())
                await asyncio.wait_for(callback_complete.wait(), timeout=1.0)

            assert task not in tracker._pending_background_tasks
            assert "Background task 'failing' failed" in caplog.text
...
def test_session_contract_log_uses_server_echo_and_bound_model_fallback():
    from apps.artagent.backend.voice.voicelive.orchestrator import (
        _session_contract_for_log,
    )

    class FakeSession:
        def as_dict(self):
            return {
                "voice": {"name": "en-GB-Sonia:DragonHDOmniLatestNeural", "temperature": 0.7, "rate": "0.97"},
    ...
    contract = _session_contract_for_log(FakeSession(), "gpt-realtime-1.5")

    assert contract["model"] == "gpt-realtime-1.5"
    assert contract["voice"]["temperature"] == 0.7
    assert contract["turn_detection"]["type"] == "azure_semantic_vad"
    assert contract["input_audio_transcription"]["model"] == "gpt-4o-transcribe"
    assert contract["input_audio_echo_cancellation"]["type"] == "server_echo_cancellation"
    assert contract["input_audio_noise_reduction"]["type"] == "azure_deep_noise_suppression"


class TestGreetingTaskCleanup:
    """Test greeting task cancellation."""

    def test_greeting_fallback_does_not_race_normal_session_update(self):
        from apps.artagent.backend.voice.voicelive.orchestrator import (
            _GREETING_FALLBACK_DELAY_SECONDS,
        )

        assert _GREETING_FALLBACK_DELAY_SECONDS >= 1.0
