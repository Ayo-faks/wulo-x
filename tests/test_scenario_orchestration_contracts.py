...
class TestVADConfigurationContracts:
    ...
    def test_vad_interrupt_flags_passed(self, unified_agent):
        """Barge-in interruption flags should be preserved in VoiceLive VAD config."""
        try:
            from azure.ai.voicelive.models import AzureSemanticVad
        except ImportError:
            pytest.skip("VoiceLive SDK not available")

        unified_agent.session["turn_detection"].update(
            {"interrupt_response": True, "auto_truncate": True, "create_response": True}
        )

        turn_detection = unified_agent.build_voicelive_vad()

        assert isinstance(turn_detection, AzureSemanticVad)
        assert turn_detection.interrupt_response is True
        assert turn_detection.auto_truncate is True
