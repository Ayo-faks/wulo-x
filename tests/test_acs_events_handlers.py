...
class TestCallEventHandlers:
    ...
    async def test_call_disconnected_finalizes_clinic_recall_ledger(
        self,
        mock_logger,
        mock_context,
        monkeypatch,
        ...
        finalized = AsyncMock()
        monkeypatch.setattr(CallEventHandlers, "_finalize_clinic_recall_ledger", finalized)
        monkeypatch.setattr(CallEventHandlers, "_close_media_connections", AsyncMock())
        monkeypatch.setattr(CallEventHandlers, "_cleanup_call_state", AsyncMock())
        monkeypatch.setattr(CallEventHandlers, "_resolve_session_id", AsyncMock(return_value=None))

        await CallEventHandlers.handle_call_disconnected(mock_context)

        finalized.assert_awaited_once_with(mock_context)
    ...
    async def test_handle_inbound_call_received(self, mock_logger, mock_context):
        ...
        mock_context.event.data = {
            "callConnectionId": "test_123",
            "from": {"kind": "phoneNumber", "phoneNumber": {"value": "+12025550116"}},
        }
        ...
        assert updates["caller_id"] == "+12025550116"
