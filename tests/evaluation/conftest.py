...
_live_evaluation_requested = os.environ.get("EVAL_USE_REAL_AOAI", "").lower() in {
    "1",
    "true",
    "yes",
}
...
if _live_evaluation_requested:
    ...
    try:
        ...
        if _appconfig_loaded:
            ...
            print(
                "✓ App Config loaded | "
                f"endpoint={status.get('endpoint', '')[:40]}... "
        ...
        print(f"⚠ App Config provider not available: {exc}", file=sys.stderr)
    except Exception as exc:
        print(f"⚠ App Config bootstrap failed: {exc}", file=sys.stderr)
...
def pytest_configure(config: pytest.Config) -> None:
    ...
    if not _live_evaluation_requested:
        return
    ...
    if "ai_foundry_project_endpoint" not in values:
        ...
        if account_endpoint and project_id:
            ...
            if "/projects/" in project_id:
                ...
                # Input: https://test-resource.cognitiveservices.azure.com/
                # Output resource name: test-resource
                import re
