...
check_regional_availability() {
    ...
    if [[ "$use_live_checks" == "true" ]]; then
        ...
        if check_openai_model_region "$location"; then
            ...
            warn "    Preference order: uksouth/ukwest, swedencentral, other Europe, then eastus2/westus2/eastus if no preferred region works"
            warnings=$((warnings + 1))
        fi
    else
    ...
    if [[ " ${voice_live_regions[*]} " =~ " ${location} " ]]; then
        ...
        if [[ -n "$configured_voice_live" ]]; then
            ...
            info "  Default is swedencentral unless you set TF_VAR_voice_live_location"
            ...
            if [[ "$voice_live_choice" =~ ^[0-9]+$ ]] && \
                ...
                info "Skipped Voice Live region selection (default: swedencentral)"
            fi
        fi
...
recommend_regions() {
    ...
    log "   • uksouth       - Preferred app/data region for UK residency"
    log "   • ukwest        - Secondary UK app/data fallback"
    log "   • swedencentral - Preferred European fallback; required for Voice Live when UK is unavailable"
    log "   • westeurope    - Other European fallback"
    log "   • eastus2       - US fallback only if preferred locations cannot support required services"
    log ""
}
