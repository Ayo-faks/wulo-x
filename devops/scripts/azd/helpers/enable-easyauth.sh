...
# Container App secret holding the Google OAuth client secret (Key Vault-sourced).
readonly GOOGLE_SECRET_NAME="google-provider-authentication-secret"
...
OPTIONS:
    ...
    --dry-run               Print the merged (redacted) auth config without calling Azure
    -h, --help              Show this help message

GOOGLE LOGIN (optional second provider):
    Set GOOGLE_AUTH_CLIENT_ID to enable Google as an additional EasyAuth
    provider. The Google client secret must already exist as the Container App
    secret '$GOOGLE_SECRET_NAME' (sourced from Key Vault;
    never passed to this script). Microsoft remains the default redirect
    provider; Google is opt-in via the frontend button.
...
get_public_app_endpoint() {
    local hostname="${EASYAUTH_PUBLIC_HOSTNAME:-${APPLICATION_GATEWAY_HOSTNAME:-}}"

    if [[ -z "$hostname" ]] && command -v azd &>/dev/null; then
        hostname=$(azd env get-value APPLICATION_GATEWAY_HOSTNAME 2>/dev/null | head -n1 || true)
    fi
...
create_app_registration() {
    ...
    local tenant_id fqdn app_endpoint callback_url public_endpoint public_callback app_id existing_app
    local redirect_uris
    ...
    public_endpoint=$(get_public_app_endpoint)
    public_callback=""
    if [[ -n "$public_endpoint" && "$public_endpoint" != "$app_endpoint" ]]; then
        public_callback="${public_endpoint}/.auth/login/aad/callback"
    fi

    redirect_uris=("$callback_url" "$LOCAL_CALLBACK")
    [[ -n "$public_callback" ]] && redirect_uris+=("$public_callback")

    log "Tenant ID: $tenant_id"
    log "App endpoint: $app_endpoint"
    log "Callback URL: $callback_url"
    [[ -n "$public_callback" ]] && log "Public callback URL: $public_callback"
...
# ============================================================================
# Step 3b (optional): Google OAuth Client Secret as Key Vault Reference
# ============================================================================

configure_google_secret() {
    # Only runs when Google login is being enabled AND a Key Vault secret URI
    # is provided. The secret value itself never passes through this script.
    # Requires GOOGLE_SECRET_IDENTITY_ID (resource ID of an identity on the
    # Container App with 'Key Vault Secrets User' on the vault).
    [[ -z "${GOOGLE_AUTH_CLIENT_ID:-}" || -z "${GOOGLE_SECRET_KV_URI:-}" ]] && return 0

    header "🔒 Step 3b: Google Client Secret (Key Vault reference)"

    if [[ -z "${GOOGLE_SECRET_IDENTITY_ID:-}" ]]; then
        fail "GOOGLE_SECRET_KV_URI is set but GOOGLE_SECRET_IDENTITY_ID is not. Both are required to wire the Key Vault reference."
    fi

    log "Setting secret '$GOOGLE_SECRET_NAME' as Key Vault reference..."
    if ! az containerapp secret set \
        --resource-group "$RESOURCE_GROUP" \
        --name "$CONTAINER_APP" \
        --secrets "${GOOGLE_SECRET_NAME}=keyvaultref:${GOOGLE_SECRET_KV_URI},identityref:${GOOGLE_SECRET_IDENTITY_ID}" \
        --output none; then
        fail "Failed to set Key Vault-referenced secret '$GOOGLE_SECRET_NAME'."
    fi

    success "Google client secret wired from Key Vault"
    footer
}
...
enable_container_app_auth() {
    ...
    local subscription_id resource_id api_version auth_config google_provider

    # Google is an env-gated second provider. Entra stays the default redirect
    # provider; the Google client secret is a pre-provisioned Container App
    # secret (Key Vault-sourced) and is never handled by this script.
    google_provider=""
    if [[ -n "${GOOGLE_AUTH_CLIENT_ID:-}" ]]; then
        log "Google provider: enabled (secret setting: $GOOGLE_SECRET_NAME)"
        if [[ "$DRY_RUN" -ne 1 ]]; then
            local google_secret_present
            google_secret_present=$(az containerapp secret list \
                --resource-group "$RESOURCE_GROUP" \
                --name "$CONTAINER_APP" \
                --query "[?name=='${GOOGLE_SECRET_NAME}'].name" \
                -o tsv 2>/dev/null || echo "")
            if [[ -z "$google_secret_present" ]]; then
                fail "GOOGLE_AUTH_CLIENT_ID is set but Container App secret '$GOOGLE_SECRET_NAME' is missing. Provision it from Key Vault first."
            fi
        fi
        google_provider=$(cat <<EOF
,
            "google": {
                "enabled": true,
                "registration": {
                    "clientId": "$GOOGLE_AUTH_CLIENT_ID",
                    "clientSecretSettingName": "$GOOGLE_SECRET_NAME"
                }
            }
EOF
)
    else
        log "Google provider: disabled (GOOGLE_AUTH_CLIENT_ID not set)"
    fi
...
EOF
    ...
    if [[ "$DRY_RUN" -eq 1 ]]; then
        log "[dry-run] Would PUT authConfigs/current with (client IDs redacted):"
        printf '%s\n' "$auth_config" | sed -E 's/("clientId": ")[^"]*/\1REDACTED/'
        footer
        return 0
    fi
...
show_summary() {
    ...
    [[ -n "${public_callback:-}" ]] && log "  • Public Redirect URI: $public_callback"
    log ""
...
main() {
    ...
    [[ "$DRY_RUN" -eq 1 ]] && log "Mode: DRY RUN (no Azure changes)"
    footer

    if [[ "$DRY_RUN" -eq 1 ]]; then
        # Offline: render the auth config that would be written, with
        # placeholders for values normally resolved from Azure.
        APP_ID="${APP_ID:-<app-registration-client-id>}"
        ISSUER="${ISSUER:-<entra-issuer-url>}"
        get_subscription_id() { echo "<subscription-id>"; }
        enable_container_app_auth
