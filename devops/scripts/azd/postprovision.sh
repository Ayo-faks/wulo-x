...
#   3. Phase 0 PostgreSQL seed data
#   4. App Config URL updates (known only after deploy)
...
task_phone_number() {
    ...
    case "$choice" in
        1)
            ...
            log ""
            read -rp "│ Phone (E.164 format, e.g. +12025550100): " phone
            if [[ "$phone" =~ ^\+[0-9]{10,15}$ ]]; then
                appconfig_set "$endpoint" "azure/acs/source-phone-number" "$phone" "$label"
                ...
                fail "Invalid format. Phone must be in E.164 format (e.g., +12025550100)"
            fi
            ;;
        *)
...
# ============================================================================
# Task 3: Phase 0 PostgreSQL Seed
# ============================================================================

task_postgres_seed_phase0() {
    header "🐘 Task 3: Phase 0 PostgreSQL Seed"

    local seed_script="$HELPERS_DIR/seed-postgres-phase0.sh"
    if [[ ! -f "$seed_script" ]]; then
        warn "seed-postgres-phase0.sh not found, skipping"
        footer
        return 0
    fi

    if AZD_LOG_IN_BOX=true bash "$seed_script"; then
        success "Phase 0 missed appointments seeded"
    else
        warn "Phase 0 Postgres seed failed"
    fi
...
task_update_urls() {
    ...
    # Determine browser-visible backend URL. When Application Gateway is enabled,
    # frontend REST calls must use the public app hostname so /api is routed by
    # the gateway; direct Container App FQDNs can be private or DNS-internal.
    ...
    [[ -z "$backend_url" ]] && backend_url=$(azd_get "BACKEND_API_URL")
    ...
    local count=0 required_count=8
    ...
    appconfig_set "$endpoint" "app/sms/twilio/webhook-base-url" "$backend_url" "$label" && ((count++)) || true
    appconfig_set "$endpoint" "app/sms/twilio/status-callback-url" "$backend_url/api/v1/sms/twilio" "$label" && ((count++)) || true
    appconfig_set "$endpoint" "app/voice/twilio/twiml-url" "$backend_url/api/v1/voice/twilio/twiml" "$label" && ((count++)) || true
    appconfig_set "$endpoint" "app/voice/twilio/status-callback-url" "$backend_url/api/v1/voice/twilio/call-status" "$label" && ((count++)) || true
    appconfig_set "$endpoint" "app/voice/twilio/recording-status-callback-url" "$backend_url/api/v1/voice/twilio/recording-status" "$label" && ((count++)) || true
    
    if [[ $count -eq $required_count ]]; then
        trigger_config_refresh "$endpoint" "$label"
        success "All URLs updated ($count/$required_count)"
    else
        warn "Some updates failed ($count/$required_count)"
        return 1
    fi
...
# ============================================================================
# Task 5: Sync Infrastructure Keys to App Configuration
# ============================================================================

task_sync_appconfig() {
    header "📦 Task 5: Sync Infrastructure Keys"
    ...
    if AZD_LOG_IN_BOX=true bash "$sync_script" --endpoint "$endpoint" --label "$label"; then
        ...
        fail "Required infrastructure keys failed to sync"
        footer
        return 1
    fi
...
# ============================================================================
# Task 5a: Configure ordinary PostgreSQL role for privacy Jobs
# ============================================================================

task_configure_privacy_db_role() {
    header "🔐 Task 5a: Clinic Recall Privacy Database Role"

    local helper="$HELPERS_DIR/configure-clinic-recall-privacy-db-role.sh"
    if [[ ! -f "$helper" ]]; then
        fail "Privacy database role helper is missing"
        footer
        return 1
    fi
    if bash "$helper"; then
        success "Privacy database role is ready"
    else
        fail "Privacy database role setup failed; rights/retention activation remains blocked"
        footer
        return 1
    fi
    footer
...
# ============================================================================
# Task 8: Enable EasyAuth for CardAPI MCP (Optional)
# ============================================================================

task_enable_easyauth_cardapi_mcp() {
    header "🔐 Task 8: CardAPI MCP Authentication (EasyAuth)"
...
main() {
    ...
    task_postgres_seed_phase0 || true
    task_configure_privacy_db_role
    task_sync_appconfig
    task_update_urls
