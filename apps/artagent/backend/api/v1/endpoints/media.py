...
async def _create_media_handler(
    ...
    if stream_mode == StreamMode.MEDIA:
        ...
        await _seed_clinic_recall_context(websocket.app.state, call_connection_id, handler.memory_manager)
        return handler
        ...
        await _seed_clinic_recall_context(websocket.app.state, call_connection_id, memory_manager)
        websocket.state.session_id = session_id
...
async def _seed_clinic_recall_context(app_state, call_connection_id: str | None, memory_manager) -> None:
    """Seed trusted Clinic Recall call context into core memory for tool injection."""
    if not call_connection_id or not memory_manager:
        return
    conn_manager = getattr(app_state, "conn_manager", None)
    if not conn_manager or not hasattr(conn_manager, "get_call_context"):
        return
    try:
        context = await conn_manager.get_call_context(call_connection_id)
    except Exception:
        logger.debug("Failed to load call context for Clinic Recall seeding", exc_info=True)
        return
    if not context or context.get("source") not in {"clinic_recall_voice_worker", "clinic_recall_inbound"}:
        return
    for key in (
        "clinic_id",
        "patient_id",
    ...
    if scenario:
        ...
        memory_manager.set_corememory("active_scenario_name", scenario)
