...
async def browser_conversation_endpoint(
    ...
    demo_token: str | None = Query(None, description="Signed public-demo token (forces demo scenario)"),
) -> None:
    """
    ...
    - demo_token: signed token from POST /api/v1/demo/session; when present the
      session is forced onto the demo scenario and hard-capped in duration.
    """
    handler: Any = None  # VoiceHandler or VoiceLiveSDKHandler
    memory_manager: MemoManager | None = None
    conn_id: str | None = None

    # Deterministic public-demo gate: fail closed before accepting the socket.
    demo_deadline_s: float | None = None
    if demo_token is not None:
        from src.clinic_recall.demo_gate import DemoGateError, demo_max_seconds

        from .demo import validate_browser_demo_token

        try:
            validate_browser_demo_token(demo_token)
        except DemoGateError as exc:
            logger.warning("Browser demo websocket rejected: %s", exc.reason)
            await websocket.close(code=1008)
            return
        scenario = "demo"
        demo_deadline_s = float(demo_max_seconds())
    ...
    async with session_context(
        ...
        try:
            ...
            if stream_mode == StreamMode.VOICE_LIVE:
                if demo_deadline_s is not None:
                    try:
                        async with asyncio.timeout(demo_deadline_s):
                            await _process_voice_live_messages(websocket, handler, session_id, conn_id)
                    except TimeoutError:
                        logger.info(
                            "[%s] Demo duration limit reached (%.0fs); closing session",
                            session_id,
                            demo_deadline_s,
                        )
                        try:
                            await websocket.close(code=1000, reason="demo_time_limit")
                        except Exception:
                            logger.debug("[%s] Demo close failed", session_id, exc_info=True)
                else:
                    await _process_voice_live_messages(websocket, handler, session_id, conn_id)
            else:
