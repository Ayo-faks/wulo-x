...
_EVENTGRID_VALIDATION_EVENT = "Microsoft.EventGrid.SubscriptionValidationEvent"
...
def _clinic_recall_call_context(raw: dict[str, Any] | None) -> dict[str, str]:
    """Extract the non-PII Clinic Recall context this backend will persist for tools."""
    if not raw or raw.get("source") != "clinic_recall_voice_worker":
        return {}
    context: dict[str, str] = {"source": "clinic_recall_voice_worker"}
    for key in ("scenario", "clinic_id", "patient_id", "outreach_job_id"):
...
def _extract_subscription_validation_code(events: list[dict[str, Any]]) -> str | None:
    for event in events:
        if _event_type(event) != _EVENTGRID_VALIDATION_EVENT:
            continue
...
async def initiate_call(
    ...
    """
    if _clinic_recall_call_context(request.context):
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Clinic Recall calls require the durable Twilio CALL worker",
        )
                ...
                if result.get("status") == "success":
                    ...
                    try:
                        ...
                        if hasattr(http_request.app.state, "conn_manager"):
                            ...
                            base_context.update(_clinic_recall_call_context(request.context))
...
async def answer_inbound_call(
    ...
    validation_code = _extract_subscription_validation_code(_as_event_list(request_body))
    if validation_code:
        logger.info("ACS incoming-call Event Grid subscription validation received")
