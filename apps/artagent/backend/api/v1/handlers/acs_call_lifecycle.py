...
from src.clinic_recall.inbound_transport import InboundCallContext, InboundRouteError
from src.clinic_recall.inbound_transport import prepare_inbound_call as prepare_clinic_inbound_call
from src.clinic_recall.models import CallRecord
from src.clinic_recall.recording import bind_call_record_provider_identity
...
class ACSLifecycleHandler:
    ...
    async def _handle_incoming_call(
        ...
        called_id = self._extract_called_id(event_data.get("to", {}))
        ...
        inbound_context = self._prepare_clinic_inbound_context(event_data, caller_id, called_id)
        recording_enabled = (
            inbound_context.record_call
            if inbound_context is not None
            else record_call
            if record_call is not None
            else ENABLE_ACS_CALL_RECORDING
        )
        ...
        if call_connection_id:
            ...
            if inbound_context is not None:
                self._bind_acs_call_record_identity(
                    inbound_context,
                    call_connection_id,
                )
                await self._persist_clinic_inbound_context(app_state, call_connection_id, inbound_context)
    ...
    def _extract_called_id(self, called_info: dict[str, Any]) -> str:
        """Extract the called ACS phone number from EventGrid data."""
        if called_info.get("kind") == "phoneNumber":
            return called_info.get("phoneNumber", {}).get("value", "unknown")
        raw = str(called_info.get("rawId") or "")
        return raw[2:] if raw.startswith("4:") else raw or "unknown"

    def _prepare_clinic_inbound_context(
        self,
        event_data: dict[str, Any],
        caller_id: str,
        called_id: str,
    ) -> InboundCallContext | None:
        if not called_id or called_id == "unknown":
            return None
        provider_call_id = str(
            event_data.get("correlationId")
            or event_data.get("serverCallId")
            or event_data.get("incomingCallContext")
            or ""
        ).strip()
        ...
        try:
            with get_sessionmaker().begin() as session:
                return prepare_clinic_inbound_call(
                    session,
                    provider=ClinicPhoneProvider.ACS,
                    provider_call_id=provider_call_id,
                    called_number=called_id,
                    caller_number=caller_id if caller_id != "unknown" else None,
                    metadata=event_data,
                    now=datetime.now(UTC),
                )
        except InboundRouteError as exc:
            logger.info("ACS inbound call has no active Clinic Recall route: %s", exc.reason)
            return None

    async def _persist_clinic_inbound_context(
        self,
        app_state,
        call_connection_id: str,
        context: InboundCallContext,
    ) -> None:
        conn_manager = getattr(app_state, "conn_manager", None) if app_state is not None else None
        if not conn_manager or not hasattr(conn_manager, "set_call_context"):
            return
        stream_parameters = context.stream_parameters()
        stream_parameters["provider_call_id"] = call_connection_id
        await conn_manager.set_call_context(call_connection_id, stream_parameters)

    def _bind_acs_call_record_identity(
        self,
        context: InboundCallContext,
        call_connection_id: str,
    ) -> None:
            ...
            with clinic_scope(session, context.clinic_id):
                record = session.execute(
                    tenant_select(CallRecord).where(
                        CallRecord.inbound_call_id == context.inbound_call_id,
                    )
                ).scalar_one_or_none()
                if record is None:
                    raise InboundRouteError(
                        "call_record_missing",
                        "ACS call ledger row is missing",
                    )
                bind_call_record_provider_identity(
                    ...
                    provider=ClinicPhoneProvider.ACS,
                    provider_call_id=call_connection_id,
                )
