...
class CallEventHandlers:
    ...
    async def handle_call_disconnected(context: CallEventContext) -> None:
        ...
        with tracer.start_as_current_span(
            ...
            await CallEventHandlers._finalize_clinic_recall_ledger(context)
    ...
    @staticmethod
    async def _finalize_clinic_recall_ledger(context: CallEventContext) -> None:
        ...
        if not conn_manager or not hasattr(conn_manager, "get_call_context"):
            return
        try:
            call_context = await conn_manager.get_call_context(context.call_connection_id)
        except Exception:
            logger.warning("ACS call context unavailable during ledger finalization")
            return
        if not call_context or call_context.get("source") != "clinic_recall_inbound":
            return
        clinic_id = str(call_context.get("clinic_id") or "").strip()
        provider_call_id = str(call_context.get("provider_call_id") or "").strip()
        if not clinic_id or not provider_call_id:
            return
