...
class VoiceLiveSettings(BaseSettings):
    ...
    voicelive_token_warmup_enabled: bool = Field(
        default=False,
        description="Prefetch only the VoiceLive AAD token during deferred startup",
    )
    voicelive_token_warmup_timeout_seconds: float = Field(
        default=5.0,
        gt=0.0,
        description="Maximum seconds for non-blocking VoiceLive token warmup",
    )
