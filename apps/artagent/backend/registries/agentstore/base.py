...
class VoiceConfig:
    ...
    rate: str = "+0%"
    pitch: str = "+0%"
    # HD voice expressiveness (0.0-1.0); None = omit from the session payload.
...
class UnifiedAgent:
        ...
        if voice_type in {"azure-standard", "azure_standard", "azure"}:
            ...
            for key in ("style", "pitch", "rate"):
                ...
                if val is not None and val != "+0%":
                    optionals[key] = val
            # HD voices accept temperature (0.0-1.0) for expressiveness; the
            # SDK AzureStandardVoice model carries it as a typed field.
            if self.voice.temperature is not None:
                optionals["temperature"] = self.voice.temperature
    ...
    def build_voicelive_vad(self) -> Any | None:
        ...
        for key in ("interrupt_response", "auto_truncate", "create_response"):
            if key in cfg:
                common_kwargs[key] = _config_bool(cfg[key])
    ...
    async def apply_voicelive_session(
        ...
        *,
        ...
        with tracer.start_as_current_span(
            ...
            # Server-side audio hygiene (agent.yaml session keys). Telephony
            # callers echo agent audio back into the inbound track; without
            # echo cancellation the echo trips VAD/STT as phantom user turns.
            echo_cfg = self.session.get("input_audio_echo_cancellation") if self.session else None
            if echo_cfg and echo_cfg.get("type"):
                kwargs["input_audio_echo_cancellation"] = AudioEchoCancellation(type=echo_cfg["type"])
            noise_cfg = self.session.get("input_audio_noise_reduction") if self.session else None
            if noise_cfg and noise_cfg.get("type"):
                kwargs["input_audio_noise_reduction"] = AudioNoiseReduction(type=noise_cfg["type"])
