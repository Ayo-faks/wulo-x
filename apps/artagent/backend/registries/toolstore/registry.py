...
def initialize_tools() -> int:
    ...
    tool_modules = [
        ...
        (
            "clinic_recall",
            lambda: __import__(
                "apps.artagent.backend.registries.toolstore.clinic_recall", fromlist=[""]
            ),
        ),
        (
            "inbound_clinic",
            lambda: __import__(
                "apps.artagent.backend.registries.toolstore.inbound_clinic", fromlist=[""]
            ),
        ),
        (
            "compliance",
    ...
    for module_name, import_func in tool_modules:
        try:
            full_module_name = f"apps.artagent.backend.registries.toolstore.{module_name}"
            was_loaded = full_module_name in sys.modules
            module = import_func()
            if was_loaded:
