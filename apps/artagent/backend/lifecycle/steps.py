...
def register_warmup_step(manager: LifecycleManager, app: FastAPI) -> None:
    ...
    async def start() -> None:
        ...
        async def warm_voicelive_token():
            try:
                from apps.artagent.backend.voice.voicelive.credentials import (
                    warm_voicelive_token as warm_token,
                )
                from apps.artagent.backend.voice.voicelive.settings import get_settings

                settings = get_settings()
                result = await warm_token(
                    settings,
                    timeout_seconds=settings.voicelive_token_warmup_timeout_seconds,
                )
                return ("voicelive_token", result)
            except Exception:
                logger.debug("VoiceLive token warmup unavailable", exc_info=True)
                return (
                    "voicelive_token",
                    {
        ...
        warmup_tasks.append(warm_voicelive_token())
        ...
        token_result = warmup_results.get("voicelive_token")
        token_result = token_result if isinstance(token_result, dict) else {}
        from utils.operational_metrics import record_warmup_metrics

        record_warmup_metrics(
            duration_ms=(time.perf_counter() - warmup_started) * 1000,
            success=bool(token_result.get("success")),
            status=str(token_result.get("status") or "unknown"),
            token_request_count=int(token_result.get("token_request_count") or 0),
        )
...
def register_mcp_servers_step(manager: LifecycleManager, app: FastAPI) -> None:
    ...
    from apps.artagent.backend.registries.toolstore.registry import register_mcp_tool
                    ...
                    if await session.connect():
                        ...
                        for tool_info in discovered_tools:
                            ...
                            def make_executor(
                                ...
                                async def execute_mcp_tool(args: dict) -> dict:
