...
is_containerapp_url() {
    case "$1" in
        *azurecontainerapps.io*) return 0 ;;
        *) return 1 ;;
    esac
}
...
if [ -n "$AZURE_APPCONFIG_ENDPOINT" ]; then
    ...
    if [ -n "$ACCESS_TOKEN" ]; then
        ...
        if [ -n "$appconfig_backend_url" ] && [ "$appconfig_backend_url" != "null" ] && [ "$appconfig_backend_url" != "https://placeholder.azurecontainerapps.io" ]; then
            if is_containerapp_url "$appconfig_backend_url"; then
                echo "⚠️  Ignoring backend-url from App Config because it points at a Container App host: $appconfig_backend_url"
                echo "   Browser REST calls will use same-origin /api routing instead"
            else
        ...
        if [ -n "$appconfig_ws_url" ] && [ "$appconfig_ws_url" != "null" ] && [ "$appconfig_ws_url" != "wss://placeholder.azurecontainerapps.io" ]; then
            if is_containerapp_url "$appconfig_ws_url"; then
                echo "⚠️  Ignoring ws-url from App Config because it points at a Container App host: $appconfig_ws_url"
                echo "   WebSocket calls will use the same public origin as REST calls"
            else
...
# Google login is opt-in per environment; default off so environments without
# a configured Google provider never render a dead sign-in button.
ENABLE_GOOGLE_LOGIN_VALUE="${ENABLE_GOOGLE_LOGIN:-false}"
echo "📝 Replacing __ENABLE_GOOGLE_LOGIN__ with: $ENABLE_GOOGLE_LOGIN_VALUE"
find /app/dist -type f -name "*.js" -exec sed -i "s|__ENABLE_GOOGLE_LOGIN__|${ENABLE_GOOGLE_LOGIN_VALUE}|g" {} \;
