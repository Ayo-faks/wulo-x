...
const ChatBubble = ({ message }) => {
  ...
  if (message?.type === "event") {
    ...
    return (
      <div style={{ width: "100%", display: "flex", justifyContent: "center", padding: "2px 12px" }}>
        <span style={{ fontSize: "11px", color: "var(--vsc-fg-muted)", marginRight: "6px" }}>•</span>
        <div style={{ display: "flex", flexWrap: "wrap", gap: "6px", alignItems: "center", justifyContent: "center", textAlign: "center", color: "var(--vsc-fg)", fontSize: "12px" }}>
          <span style={{ fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", color: palette }}>
            {eventLabel}
          </span>
          {timestampLabel && (
            <span style={{ color: "var(--vsc-fg-muted)", fontFamily: 'Roboto Mono, ui-monospace, Menlo, Consolas, "Courier New", monospace', letterSpacing: "0.02em" }}>
              {timestampLabel}
            </span>
          )}
          {detailText && (
            <span style={{ color: "var(--vsc-fg)", whiteSpace: "pre-wrap" }}>
              {detailText}
            </span>
          )}
        </div>
      ...
      <Box sx={{ width: "100%", display: "flex", justifyContent: "center", px: 1, py: 1 }}>
        <Card
          ...
          sx={{
            ...
            maxWidth: 600,
            borderRadius: 3,
            background: "linear-gradient(135deg, rgba(241, 76, 76, 0.24), var(--vsc-error))",
        ...
        >
          <CardHeader
            avatar={<ErrorOutlineRoundedIcon sx={{ color: "rgba(241, 76, 76, 0.16)", fontSize: 28 }} />}
            title={
  ...
  if (isTool) {
    ...
    const cardGradient = isFailure
      ? "linear-gradient(135deg, var(--vsc-error), var(--vsc-error))"
      : isSuccess
      ...
      <Box sx={{ width: "100%", display: "flex", justifyContent: "center", px: 1, py: 1 }}>
        <Card
          ...
          sx={{
            ...
            color: "var(--vsc-fg-strong)",
            border: "1px solid rgba(255,255,255,0.16)",
            boxShadow: "0 18px 40px rgba(var(--vsc-accent-rgb),0.28)",
          }}
        >
          <CardHeader
            avatar={<BuildCircleRoundedIcon sx={{ color: "rgba(var(--vsc-accent-rgb), 0.16)" }} />}
            title={
              <Typography variant="subtitle1" sx={{ fontWeight: 600, letterSpacing: 0.4 }}>
                {toolName}
              </Typography>
            ...
            action={
              <Chip
                ...
                icon={chipIcon}
                sx={{
                  color: chipColor === "success" ? "var(--vsc-success)" : chipColor === "error" ? "var(--vsc-error)" : "var(--vsc-fg)",
                  borderColor: "rgba(248,250,252,0.4)",
                  backgroundColor: "rgba(248,250,252,0.15)",
                  '& .MuiChip-icon': {
                    color: chipColor === "success" ? "var(--vsc-success)" : chipColor === "error" ? "var(--vsc-error)" : "var(--vsc-fg)",
                ...
                <Box sx={{ mb: detailText ? 2 : 0 }}>
                  <LinearProgress
                    ...
                    sx={{
                      ...
                      '& .MuiLinearProgress-bar': { backgroundColor: "var(--vsc-panel-bg)" },
                    }}
                  />
                </Box>
              )}
  ...
  if (isSystem) {
    ...
    return (
      <div style={{ width: "100%", display: "flex", justifyContent: "center", padding: "2px 12px" }}>
        <span style={{ fontSize: "11px", color: "var(--vsc-fg-muted)", marginRight: "6px" }}>•</span>
        <div style={{ display: "flex", flexWrap: "wrap", gap: "6px", alignItems: "center", justifyContent: "center", textAlign: "center", fontSize: "12px", color: "var(--vsc-fg)" }}>
