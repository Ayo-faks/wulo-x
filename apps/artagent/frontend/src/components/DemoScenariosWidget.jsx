...
const styles = {
  ...
  toggleButton: (open) => ({
    ...
    background: open
      ...
      : 'linear-gradient(135deg, var(--vsc-fg), var(--vsc-fg))',
  ...
  filterButton: (active) => ({
    ...
    alignItems: 'center',
    gap: '6px',
    color: active ? 'var(--vsc-fg)' : 'var(--vsc-border)',
    background: active ? 'var(--vsc-link)' : 'rgba(248, 250, 252, 0.08)',
    ...
    <div key={index} style={styles.scenarioCard}>
      ...
      {(scenario.sections || []).map((section) => (
        <div key={section.label} style={{ marginBottom: '12px' }}>
          <div style={{ fontSize: '11px', color: 'var(--vsc-link)', fontWeight: 600, textTransform: 'uppercase' }}>
            {section.label}
          </div>
  ...
  const renderPanel = () => (
    ...
    >
      ...
      <div style={styles.filterBar}>
        <div style={{ fontSize: '10px', color: 'var(--vsc-fg)', fontWeight: 600, textTransform: 'uppercase' }}>
          Filter by stack
        </div>
