...
const TAB_META = {
  verification: { icon: '🛡️', accent: 'var(--vsc-accent)' },
  identity: { icon: '🪪', accent: 'var(--vsc-link)' },
  banking: { icon: '🏦', accent: 'var(--vsc-accent)' },
  transactions: { icon: '💳', accent: 'var(--vsc-accent)' },
  contact: { icon: '☎️', accent: 'var(--vsc-success)' },
  // Insurance scenario tabs
  policies: { icon: '📋', accent: 'var(--vsc-success)' },
  claims: { icon: '📝', accent: 'var(--vsc-warning)' },
};
...
const ProfileDetailRow = ({ icon, label, value, multiline = false }) => (
  <Box sx={{
    ...
    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
      {icon && <Box sx={{ color: 'var(--vsc-fg-muted)', fontSize: '14px' }}>{icon}</Box>}
      <Typography sx={{
        fontWeight: 600,
        color: 'var(--vsc-fg-muted)',
        fontSize: '11px',
          ...
          <>
              ...
              <SectionCard sx={{ mt: 2 }}>
                ...
                {activeAlerts.map((alert, idx) => (
                  <Box key={idx} sx={{
                    ...
                    <Typography sx={{ fontSize: '11px', fontWeight: 700, color: 'var(--vsc-fg)', mb: 0.5 }}>
                      {alert.priority === 'high' ? '🔴' : '🟡'} {toTitleCase(alert.type)}
                    </Typography>
                    <Typography sx={{ fontSize: '11px', color: 'var(--vsc-fg-muted)', mb: 0.5 }}>
                      {alert.message}
                    </Typography>
                    <Typography sx={{ fontSize: '10px', color: 'var(--vsc-fg-muted)', fontWeight: 600 }}>
                      Action: {alert.action}
                    </Typography>
                  </Box>
                ))}
            ...
            {(financialGoals.length > 0 || lifeEvents.length > 0 || suggestedTalkingPoints.length > 0) && (
              <SectionCard sx={{ mt: 2 }}>
                ...
                {lifeEvents.length > 0 && (
                  <>
                    <Typography sx={{ fontSize: '11px', fontWeight: 700, color: 'var(--vsc-fg-muted)', mb: 1 }}>
                      Recent Life Events
                    </Typography>
                    {lifeEvents.map((event, idx) => (
                      <Box key={idx} sx={{ mb: 1 }}>
                        ...
                        {event.details && (
                          <Typography sx={{ fontSize: '10px', color: 'var(--vsc-fg-muted)', ml: 2, mt: 0.5 }}>
                            {event.details}
                          </Typography>
                        )}
                      </Box>
                ...
                {suggestedTalkingPoints.length > 0 && (
                  <>
                    ...
                    {suggestedTalkingPoints.slice(0, 5).map((point, idx) => (
                      <Typography key={idx} sx={{ fontSize: '11px', color: 'var(--vsc-fg)', mb: 0.5, pl: 2 }}>
                        • {point}
                      </Typography>
                    ))}
                  </>
          ...
          <>
            <SectionCard>
              ...
              {transactions.length ? (
                    ...
                    return (
                      <Box
                        ...
                        sx={{
                          ...
                          borderRadius: '14px',
                          padding: '12px 14px',
                          backgroundColor: hasFee ? 'rgba(254, 242, 242, 0.5)' : 'var(--vsc-fg-strong)',
                          boxShadow: '0 6px 16px rgba(15,23,42,0.08)',
                        }}
                      >
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 0.5 }}>
                          <Box sx={{ flex: 1 }}>
                            <Typography sx={{ fontWeight: 700, color: 'var(--vsc-fg)', fontSize: '13px' }}>
                              {isInternational && '🌍 '}{txn.merchant}
                            </Typography>
                            <Typography sx={{ color: 'var(--vsc-fg-muted)', fontSize: '10px', mt: 0.3 }}>
                              📍 {locationStr}
                            </Typography>
                          </Box>
                          <Box sx={{ textAlign: 'right' }}>
                            <Typography sx={{ fontWeight: 700, color: 'var(--vsc-fg)', fontSize: '12px' }}>
                              {formatCurrencyWithCents(txn.amount)}
                            </Typography>
                            {txn.original_currency && txn.original_currency !== 'USD' && (
                              <Typography sx={{ color: 'var(--vsc-fg-muted)', fontSize: '9px', fontStyle: 'italic' }}>
                        ...
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 1 }}>
                          <Typography sx={{ color: 'var(--vsc-fg-muted)', fontSize: '10px' }}>
                            {formatDateTime(txn.timestamp)} • {toTitleCase(txn.category)}
                          </Typography>
                          <Typography sx={{ color: 'var(--vsc-fg-muted)', fontSize: '9px', fontWeight: 600 }}>
                            Card ****{txn.card_last4}
                          </Typography>
                        </Box>
                        ...
                        {hasFee && (
                          <Box sx={{
                            ...
                            <Typography sx={{ color: 'var(--vsc-error)', fontSize: '10px', fontWeight: 700 }}>
                              ⚠️ {txn.fee_reason || 'Foreign Transaction Fee'}
                            </Typography>
                            <Typography sx={{ color: 'var(--vsc-error)', fontSize: '11px', fontWeight: 700 }}>
                        ...
                        {txn.status && (
                          <Typography sx={{
                            color: txn.status === 'declined' ? 'var(--vsc-error)' : 'var(--vsc-fg-muted)',
                            fontSize: '10px',
                            fontWeight: 600,
                            mt: 0.5
                          }}>
                        ...
                        {txn.notes && (
                          <Typography sx={{ color: 'var(--vsc-fg-muted)', fontSize: '9px', mt: 0.5, fontStyle: 'italic' }}>
                            Note: {txn.notes}
                          </Typography>
                        )}
                      </Box>
                ...
                <Typography sx={{ fontSize: '11px', color: 'var(--vsc-fg-muted)' }}>
                    ...
                    return (
                      ...
                      >
                        ...
                        {policy.coverage_limits && (
                          <>
                            <Divider sx={{ my: 1 }} />
                            <Typography sx={{ fontSize: '10px', fontWeight: 700, color: 'var(--vsc-fg-muted)', mb: 0.5 }}>
                              Coverage Limits
                            </Typography>
                        ...
                        {policy.vehicles && policy.vehicles.length > 0 && (
                          <>
                            ...
                            {policy.vehicles.map((vehicle, vIdx) => (
                              <Typography key={vIdx} sx={{ fontSize: '11px', color: 'var(--vsc-fg)', mb: 0.3 }}>
                  ...
                  {claims.map((claim, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <Typography sx={{ fontSize: '11px', color: 'var(--vsc-fg-muted)', mt: 0.5, mb: 1 }}>
                          {claim.description}
                        </Typography>
                        ...
                        {hasSubro && (
                          <Box sx={{
                            ...
                            <Typography sx={{ fontSize: '10px', fontWeight: 700, color: 'var(--vsc-warning)', mb: 0.5 }}>
                              📨 Subrogation Demand
                            </Typography>
  ...
  const panel = (
    <Box
      sx={{
        ...
        height: '100vh',
        maxHeight: '100vh',
        background: 'linear-gradient(180deg, var(--vsc-panel-bg), var(--vsc-panel-bg))',
        borderRadius: '0px',
        border: '1px solid rgba(226,232,240,0.9)',
      ...
      />
        {renderContent && (
          <>
            <Box sx={{
              padding: '18px',
              background: 'linear-gradient(150deg, rgba(var(--vsc-accent-rgb), 0.14) 0%, rgba(238,242,255,0.7) 40%, var(--vsc-panel-bg) 100%)',
              ...
              <Box
                sx={{
                  position: 'absolute',
                  inset: 0,
                  background: 'radial-gradient(circle at top right, rgba(var(--vsc-accent-rgb),0.25), transparent 45%)',
                  pointerEvents: 'none',
                }}
              />
              <Box sx={{ position: 'relative', zIndex: 1 }}>
                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.2 }}>
                    <Typography variant="h6" sx={{ fontSize: '18px', fontWeight: 800, color: 'var(--vsc-fg)', letterSpacing: '-0.02em' }}>
                      Profile Details
                    </Typography>
                    {expiresAt && (
                      <SummaryStat
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                  ...
                  <Box sx={{ flex: 1 }}>
                  <Typography variant="h6" sx={{ fontSize: '16px', fontWeight: 800, color: 'var(--vsc-fg)', lineHeight: 1.1 }}>
                    {data?.full_name || 'Demo User'}
                  </Typography>
                  {data?.contact_info?.email && (
                    <Typography sx={{ fontSize: '11px', color: 'var(--vsc-fg-muted)', fontWeight: 600 }}>
                      {data.contact_info.email}
                    </Typography>
                  )}
                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.8, mt: 1 }}>
                      <Chip
                        ...
                        sx={{
                          ...
                          color: tier?.toLowerCase() === 'platinum' ? 'var(--vsc-fg)' : 'var(--vsc-fg)',
                          fontSize: '10px',
                          fontWeight: 700,
                          height: '22px',
                        }}
                      ...
                      <Chip
                        ...
                        sx={{
                          ...
                          color: isInsuranceScenario ? 'var(--vsc-warning)' : 'var(--vsc-accent)',
                          fontSize: '10px',
                          fontWeight: 700,
                          height: '22px',
                        }}
            ...
            >
              ...
              >
                {visibleTabs.map((tab) => {
                  ...
                  const { icon = '•', accent = 'var(--vsc-accent)' } = TAB_META[tab.id] || {};
                  return (
                    <Box
                      key={tab.id}
                      component="button"
                      ...
                      sx={{
                        ...
                        color: isActive ? 'var(--vsc-fg-strong)' : 'var(--vsc-fg)',
                        fontSize: '11px',
                        fontWeight: 600,
