  ...
  return (
    <div style={containerStyle}>
      <svg width="100%" height={height} viewBox={`0 0 ${viewWidth} ${height}`} preserveAspectRatio="xMidYMid meet">
        <defs>
          ...
          <marker id="arrow-strong" markerWidth="6" markerHeight="6" refX="3" refY="3" orient="auto" markerUnits="strokeWidth">
            <path d="M0,0 L6,3 L0,6 z" fill="var(--vsc-warning)" />
          </marker>
        ...
        {nodes.map((node) => {
          ...
          const palette = node.id === "System"
            ? { fill: "linear-gradient(135deg, var(--vsc-panel-bg), var(--vsc-input-bg))", stroke: "var(--vsc-border)", fg: "var(--vsc-fg-muted)" }
            : node.id === "User"
            ? { fill: "linear-gradient(135deg, rgba(var(--vsc-accent-rgb), 0.18), rgba(var(--vsc-accent-rgb), 0.10))", stroke: "var(--vsc-accent)", fg: "var(--vsc-link)" }
            : node.id.includes("-tool-")
            ? { fill: "linear-gradient(135deg, rgba(204, 167, 0, 0.18), rgba(204, 167, 0, 0.08))", stroke: "var(--vsc-warning)", fg: "var(--vsc-warning)" }
            : { fill: "linear-gradient(135deg, rgba(78, 201, 176, 0.18), rgba(var(--vsc-accent-rgb), 0.08))", stroke: "var(--vsc-success)", fg: "var(--vsc-success)" };
      ...
      {selectedNode && (
        ...
        >
          <div style={{ fontSize: "11px", fontWeight: 700, color: "var(--vsc-fg)", marginBottom: "6px" }}>
          ...
          {recent
            ...
            .slice(-10)
            .map((evt, idx) => (
              <div key={`${selectedNode}-evt-${idx}`} style={{ fontSize: "11px", color: "var(--vsc-fg)", marginBottom: "4px", display: "flex", gap: "6px", alignItems: "center", flexWrap: "wrap" }}>
                {(() => {
                  ...
                  return (
                    <>
                      <span style={{ fontWeight: 700, color: "var(--vsc-accent)" }}>{label}</span>
                      <span style={{ color: "var(--vsc-fg-muted)" }}>{formatStatusTimestamp(evt.ts) || ""}</span>
                    </>
                  );
                })()}
                <span style={{ color: "var(--vsc-fg)", whiteSpace: "normal", wordBreak: "break-word" }}>{evt.text || evt.summary || evt.detail || evt.tool || ""}</span>
              </div>
            ))}
        </div>
      )}
