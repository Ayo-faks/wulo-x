  ...
  return (
    <Box
      sx={{
        ...
        background: 'linear-gradient(145deg, var(--vsc-panel-bg), var(--vsc-panel-bg))',
    ...
    >
      ...
      >
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          {icon}
          <Typography sx={{ fontWeight: 700, color: 'var(--vsc-fg)', fontSize: '12px', letterSpacing: '0.5px' }}>
            {title}
          </Typography>
          {alert && (
            <Chip
...
const MetricRow = ({ label, value, unit, trend, severity = 'info' }) => {
  ...
  return (
    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', py: 0.5, gap: 1 }}>
      <Typography sx={{ color: 'var(--vsc-fg-muted)', fontWeight: 600, fontSize: '11px' }}>{label}</Typography>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
        ...
        {unit && (
          <Typography sx={{ color: 'var(--vsc-fg-muted)', fontSize: '10px', fontWeight: 600 }}>
            {unit}
          </Typography>
        )}
        {trend && (
          <TrendingUpRoundedIcon
            sx={{
              fontSize: 12,
              color: trend > 0 ? 'var(--vsc-error)' : 'var(--vsc-success)',
  ...
  return (
    <Box sx={{ mb: 1 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 0.5 }}>
        <Typography sx={{ fontSize: '11px', fontWeight: 600, color: 'var(--vsc-fg-muted)' }}>
          {entry?.stage || 'unknown'}
        </Typography>
        <Typography
          sx={{
      ...
      <Box sx={{ position: 'relative', height: 6, bgcolor: 'var(--vsc-fg)', borderRadius: 3 }}>
        <Box
          sx={{
            position: 'absolute',
            left: 0,
  ...
  return createPortal(
    ...
    >
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', p: 2, pb: 1.5, cursor: 'grab' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          <SpeedRoundedIcon sx={{ color: 'var(--vsc-link)' }} fontSize="small" />
          <Box>
            <Typography sx={{ fontWeight: 800, fontSize: '15px', color: 'var(--vsc-fg)' }}>Session Performance (Beta)</Typography>
            <Typography sx={{ fontSize: '11px', color: 'var(--vsc-fg-muted)' }}>Real-time latency and bottleneck analysis</Typography>
          </Box>
        </Box>
      ...
      >
        ...
        <PanelCard
          ...
          icon={<RecordVoiceOverRoundedIcon sx={{ fontSize: 16, color: 'var(--vsc-link)' }} />}
        >
          ...
          {performanceData?.visitedAgents?.length > 0 && (
            <Box sx={{ mt: 1 }}>
              <Typography sx={{ fontSize: '11px', fontWeight: 700, color: 'var(--vsc-fg-muted)', mb: 0.5 }}>
        ...
        {activeScenarioHandoffs.length > 0 && (
          <PanelCard
            ...
            icon={<SwapCallsRoundedIcon sx={{ fontSize: 16, color: 'var(--vsc-accent)' }} />}
            collapsible
            defaultOpen={true}
          >
                ...
                return (
                  ...
                  >
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                      <Chip
                        ...
                        sx={{
                          fontSize: '10px',
                          height: 18,
                          bgcolor: 'rgba(var(--vsc-accent-rgb), 0.14)',
                          color: 'var(--vsc-link)',
                          fontWeight: 600,
                        }}
                      />
                      <Typography sx={{ fontSize: '10px', color: 'var(--vsc-fg-muted)' }}>→</Typography>
                      <Chip
                        label={toAgent}
                        size="small"
                        sx={{
                        ...
                        {handoffType && (
                          <Chip
                            ...
                            sx={{
                              fontSize: '9px',
                              height: 16,
                              bgcolor: handoffType === 'discrete' ? 'rgba(var(--vsc-accent-rgb), 0.18)' : 'rgba(var(--vsc-accent-rgb), 0.14)',
                              color: handoffType === 'discrete' ? 'var(--vsc-link)' : 'var(--vsc-accent)',
                              fontWeight: 500,
                            }}
                          />
                        )}
                    ...
                    {condition ? (
                      ...
                      <Typography sx={{ fontSize: '10px', color: 'var(--vsc-fg-muted)', fontStyle: 'italic' }}>
        ...
        <PanelCard
          title="Token Usage"
          icon={<MemoryRoundedIcon sx={{ fontSize: 16, color: 'var(--vsc-accent)' }} />}
        >
          <MetricRow
            label="Input Tokens"
        ...
        {latencyBreakdown.length > 0 && (
          <PanelCard
            ...
            icon={<TimelineRoundedIcon sx={{ fontSize: 16, color: 'var(--vsc-success)' }} />}
          ...
          >
            ...
            <Box sx={{ mt: 1.5 }}>
              ...
              {latencyBreakdown.map((entry) => (
                <Box key={entry.stage} sx={{ mb: 1, p: 1, bgcolor: 'var(--vsc-panel-bg)', borderRadius: '8px' }}>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                    <Typography sx={{ fontSize: '11px', fontWeight: 600, color: 'var(--vsc-fg)' }}>
                      {entry.stage}
                    </Typography>
                    <Chip
                      label={`${entry.count}x`}
        ...
        <PanelCard
          ...
          icon={<TrendingUpRoundedIcon sx={{ fontSize: 16, color: 'var(--vsc-warning)' }} />}
          collapsible
          defaultOpen={insights.length > 0}
        >
          {insights.length === 0 ? (
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, p: 1, bgcolor: 'rgba(78, 201, 176, 0.14)', borderRadius: '8px' }}>
              <Typography sx={{ fontSize: '12px', color: 'var(--vsc-success)', fontWeight: 600 }}>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
              {insights.map((issue, idx) => (
                <Box
                  ...
                  sx={{
                    p: 1,
                    bgcolor: issue.severity === 'error' ? 'rgba(241, 76, 76, 0.14)' : 'rgba(204, 167, 0, 0.14)',
                    borderRadius: '8px',
                    border: `1px solid ${issue.severity === 'error' ? 'rgba(241, 76, 76, 0.18)' : 'rgba(204, 167, 0, 0.20)'}`,
                  }}
                >
                  <Typography
                    sx={{
                      fontSize: '11px',
                      color: issue.severity === 'error' ? 'var(--vsc-error)' : 'var(--vsc-warning)',
                      fontWeight: 600
                    }}
                  >
                    {issue.message}
          ...
          <Box sx={{ mt: 1.5, p: 1, bgcolor: 'var(--vsc-fg)', borderRadius: '8px' }}>
            <Typography sx={{ fontSize: '11px', fontWeight: 600, color: 'var(--vsc-fg-muted)', mb: 0.5 }}>
              💡 Optimization Tips
            </Typography>
            <Typography sx={{ fontSize: '10px', color: 'var(--vsc-fg-muted)', lineHeight: 1.4 }}>
