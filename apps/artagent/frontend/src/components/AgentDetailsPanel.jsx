...
const SummaryRow = ({ label, value }) => (
  <Box sx={{ display: 'flex', justifyContent: 'space-between', py: 0.5, gap: 1 }}>
    ...
    <Typography sx={{ color: 'var(--vsc-fg)', fontWeight: 600, fontSize: '12px', textAlign: 'right', flex: 1 }}>
      {value || '—'}
    </Typography>
  </Box>
);
  ...
  return createPortal(
    ...
    >
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', p: 2, pb: 1.5, cursor: 'grab' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          <SmartToyRoundedIcon sx={{ color: 'var(--vsc-link)' }} fontSize="small" />
          <Box>
            <Typography sx={{ fontWeight: 800, fontSize: '15px', color: 'var(--vsc-fg)' }}>Agent Details</Typography>
            <Typography sx={{ fontSize: '11px', color: 'var(--vsc-fg-muted)' }}>Debug context for current session</Typography>
          </Box>
        </Box>
      ...
      >
        <PanelCard
          title="Active Agent"
          icon={<SmartToyRoundedIcon sx={{ fontSize: 16, color: 'var(--vsc-link)' }} />}
        >
          <Typography sx={{ fontSize: '18px', fontWeight: 800, color: 'var(--vsc-fg)', mb: 0.5 }}>
            {displayAgentName || 'Unknown'}
          </Typography>
          {(displayDescription || sessionConfig?.greeting) && (
            <Typography sx={{ fontSize: '12px', color: 'var(--vsc-fg-muted)', mb: 1 }}>
              {displayDescription || sessionConfig?.greeting}
            </Typography>
          )}
          <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, mb: 1 }}>
            ...
            {sessionId && (
              <Chip
                ...
                sx={{
                  background: 'linear-gradient(135deg, rgba(var(--vsc-accent-rgb),0.08), rgba(79,70,229,0.06))',
                  color: 'var(--vsc-accent)',
                  fontWeight: 600,
                  border: '1px solid rgba(var(--vsc-accent-rgb),0.2)',
                  fontSize: '11px',
                }}
              />
            )}
        ...
        <PanelCard
          ...
          icon={<BuildRoundedIcon sx={{ fontSize: 16, color: 'var(--vsc-link)' }} />}
          collapsible
          defaultOpen={true}
        >
          {displayTools.length === 0 ? (
            <Typography sx={{ fontSize: '12px', color: 'var(--vsc-fg-muted)' }}>
                ...
                return (
                  <Chip
                    ...
                    sx={{
                      ...
                      color: isHandoff ? 'var(--vsc-warning)' : 'var(--vsc-link)',
                    }}
                    variant="outlined"
                  />
                );
        ...
        <PanelCard
          title="Latest Turns"
          icon={<TimelineRoundedIcon sx={{ fontSize: 16, color: 'var(--vsc-accent)' }} />}
          collapsible
          defaultOpen={false}
        >
        ...
        <PanelCard
          ...
          icon={<TimelineRoundedIcon sx={{ fontSize: 16, color: 'var(--vsc-link)' }} />}
          collapsible
          defaultOpen={false}
        >
          ...
          {recentMessages.map((msg, idx) => {
            ...
            const tone = speaker === "User" ? "var(--vsc-accent)" : speaker === "System" ? "var(--vsc-fg-muted)" : "var(--vsc-link)";
            return (
              <Box
              ...
              >
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 0.5 }}>
                  ...
                  />
                  {msg.turnId && (
                    <Typography sx={{ fontSize: '10px', color: 'var(--vsc-fg-muted)' }}>
                      {msg.turnId}
                    </Typography>
                  )}
                </Box>
                <Typography sx={{ fontSize: '12px', color: 'var(--vsc-fg)', whiteSpace: 'pre-wrap' }}>
        ...
        <PanelCard
          title="Recent Tools"
          icon={<BuildRoundedIcon sx={{ fontSize: 16, color: 'var(--vsc-success)' }} />}
        >
          {toolRows.length === 0 && (
            <Typography sx={{ fontSize: '12px', color: 'var(--vsc-fg-muted)' }}>No tools invoked yet.</Typography>
          )}
          {toolRows.map((tool) => (
            <Box
            ...
            >
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                <BuildRoundedIcon sx={{ fontSize: 16, color: 'var(--vsc-success)' }} />
                <Typography sx={{ fontWeight: 700, fontSize: '12px', color: 'var(--vsc-fg)' }}>
                  {tool.tool || tool.name || 'Tool'}
                </Typography>
              </Box>
              <Typography sx={{ fontSize: '11px', color: 'var(--vsc-fg-muted)', mt: 0.5 }}>
        ...
        <PanelCard
          ...
          icon={<PersonRoundedIcon sx={{ fontSize: 16, color: 'var(--vsc-warning)' }} />}
        >
        ...
        {sessionConfig && (
          ...
          >
            ...
            <Box sx={{ mt: 1 }}>
              ...
              <Typography sx={{ fontSize: '11px', color: 'var(--vsc-fg)', whiteSpace: 'pre-wrap' }}>
