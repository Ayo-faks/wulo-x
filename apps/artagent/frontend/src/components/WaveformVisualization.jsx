...
const WaveformVisualization = React.memo(({ activeSpeaker, audioLevelRef, outputAudioLevelRef, bargeInActive = false }) => {
  ...
  const generateMultipleWaves = () => {
    ...
    if (bothDisplayActive) {
      ...
      baseColor = "var(--vsc-error)";
    } else if (assistantDisplayActive) {
      baseColor = "var(--vsc-link)";
    } else {
      baseColor = "var(--vsc-accent)";
      opacity = 0.45;
    }

    if (waveRenderState.amplitude <= 0.8) {
      baseColor = "var(--vsc-fg)";
      waves.push(
        <line
          key="wave-idle"
          x1="0"
  ...
  return (
    <div style={styles.waveformContainer} ref={containerRef}>
      <svg style={styles.waveformSvg} viewBox={`0 0 ${Math.max(canvasWidth, 200)} 110`} preserveAspectRatio="xMidYMid meet">
        <defs>
          <linearGradient id="waveGradientBarge" x1="0%" y1="0%" x2="100%" y2="0%">
            <stop offset="0%" stopColor="var(--vsc-error)" />
            <stop offset="50%" stopColor="var(--vsc-accent)" />
            <stop offset="100%" stopColor="var(--vsc-link)" />
          </linearGradient>
        </defs>
      ...
      <div style={{
        ...
        color: 'var(--vsc-fg)',
        whiteSpace: 'nowrap',
        background: 'linear-gradient(135deg, rgba(var(--vsc-accent-rgb),0.22), rgba(var(--vsc-accent-rgb),0.12))',
        padding: '4px 10px',
        borderRadius: '10px',
        border: '1px solid rgba(var(--vsc-accent-rgb),0.34)',
        boxShadow: '0 10px 22px rgba(0,0,0,0.34), inset 0 1px 0 rgba(255,255,255,0.08)',
