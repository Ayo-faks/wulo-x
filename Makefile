...
############################################################
# Clinic Recall data plane (Phase 1) - Alembic migrations
# Schema source of truth: src/clinic_recall/models.py
############################################################
ALEMBIC = $(UV_BIN) run alembic -c infra/postgres/alembic.ini

# Apply all migrations (creates schema + RLS, reconciles Phase 0 seed).
db_upgrade:
...
# Create a new (empty) revision: make db_revision MESSAGE="add x"
db_revision:
	$(ALEMBIC) revision -m "$(MESSAGE)"

# Run only the Clinic Recall data-plane unit tests (offline; no DB required).
test_clinic_recall:
	$(PYTHON_INTERPRETER) -m pytest tests/test_clinic_recall_*.py -v

# Regenerate service-specific Python image locks and fail on drift.
validate_python_image_locks:
	UV_BIN="$(UV_BIN)" bash devops/scripts/validate-python-image-locks.sh

############################################################
# AgentOps gate runners (ISOLATED env — never the app .venv/uv.lock)
# ASSERT + Red Team runners are pinned in devops/agentops/requirements-gate.txt (the
# same spec CI installs) and live in a dedicated venv so the app's pinned
# OpenTelemetry stack stays untouched. NOTE: the Red Team runner pulls
# PyRIT->base2048 (Rust); on arm64 without a prebuilt wheel install rustup first.
############################################################
AGENTOPS_GATE_VENV ?= .venv-agentops
AGENTOPS_ACCELERATOR ?= agentops-accelerator[foundry,agent]==0.5.2

# Build/refresh the isolated AgentOps gate environment (mirrors the CI install).
agentops_gate_env:
	$(UV_BIN) venv $(AGENTOPS_GATE_VENV)
	$(UV_BIN) pip install --python $(AGENTOPS_GATE_VENV) "$(AGENTOPS_ACCELERATOR)" -r devops/agentops/requirements-gate.txt
	@echo "Gate env ready: $(AGENTOPS_GATE_VENV) — run: source $(AGENTOPS_GATE_VENV)/bin/activate && agentops assert run"

RECALL_AGENT_GATE_ARGS ?=
INBOUND_ASSISTANT_GATE_ARGS ?=
CLINIC_RECALL_EVAL_PIPELINE_ARGS ?=

# Pin non-secret Foundry aliases + PyRIT OPENAI_CHAT_* aliases into .env/azd env.
agentops_hydrate_env:
	bash devops/agentops/hydrate-gate-env.sh

# Repeatable hosted Foundry eval + safety gate. Pass extra flags with
# RECALL_AGENT_GATE_ARGS="--doctor-nonblocking" or "--eval-only".
recall_agent_gate:
	bash devops/agentops/run-recall-agent-gate.sh $(RECALL_AGENT_GATE_ARGS)

recall_agent_eval:
	bash devops/agentops/run-recall-agent-gate.sh --eval-only $(RECALL_AGENT_GATE_ARGS)

inbound_assistant_gate:
	bash devops/agentops/run-recall-agent-gate.sh --agent-dir src/inbound-assistant --config agentops-inbound.yaml $(INBOUND_ASSISTANT_GATE_ARGS)

inbound_assistant_eval:
	bash devops/agentops/run-recall-agent-gate.sh --agent-dir src/inbound-assistant --config agentops-inbound.yaml --eval-only $(INBOUND_ASSISTANT_GATE_ARGS)

clinic_recall_eval_pipeline:
	bash devops/agentops/run-clinic-recall-eval-pipeline.sh $(CLINIC_RECALL_EVAL_PIPELINE_ARGS)

# Fetch per-row Foundry hosted-eval results for RCA (requires `az login`).
# Pass FETCH_EVAL_ROWS_ARGS="--results-dir .agentops/results/<ts>" for older runs.
fetch_eval_rows:
	python3 devops/agentops/fetch_foundry_instance_results.py $(FETCH_EVAL_ROWS_ARGS)

# Deterministic per-turn voice latency eval from Log Analytics AppTraces
# (requires `az login`). Pass VOICE_LATENCY_EVAL_ARGS="--input rows.json" for
# offline mode or "--workspace <id> --start <iso> --end <iso>" for live mode.
VOICE_LATENCY_EVAL_ARGS ?=
voice_latency_eval:
	python3 devops/agentops/voice_latency_eval.py $(VOICE_LATENCY_EVAL_ARGS)

.PHONY: agentops_gate_env agentops_hydrate_env recall_agent_gate recall_agent_eval inbound_assistant_gate inbound_assistant_eval clinic_recall_eval_pipeline fetch_eval_rows voice_latency_eval
...
# Usage: make set_phone_number PHONE=+12025550100
# Usage: make set_phone_number PHONE=+12025550100 APPCONFIG_ENDPOINT=appconfig-xxx.azconfig.io APPCONFIG_LABEL=dev
set_phone_number:
	...
	@if [ -z "$(PHONE)" ]; then \
		...
		echo ""; \
	        echo "Usage: make set_phone_number PHONE=+12025550100"; \
		echo ""; \
		exit 1; \
	fi
	@if [ -z "$(APPCONFIG_ENDPOINT)" ]; then \
		...
		echo ""; \
	        echo "Usage: make set_phone_number PHONE=+12025550100 APPCONFIG_ENDPOINT=appconfig-xxx.azconfig.io"; \
		exit 1; \
	fi
...
help:
	...
	@echo "  set_phone_number                 Set ACS phone number in App Config (PHONE=+12025550100)"
	...
	@echo "  PHONE                            Phone number for testing (example: +12025550101)"
	@echo ""
...
PITCH_DECK_SRC = artifacts/pitch-deck/wulo-x-clinic-recall.md
# Marp needs a Chromium/Chrome for PDF export; reuse the Playwright browser if present
CHROME_PATH ?= $(shell ls -d $(HOME)/.cache/ms-playwright/chromium-*/chrome-linux/chrome 2>/dev/null | sort | tail -1)

# Render the Clinic Recall pitch deck to PDF + HTML via Marp CLI
pitch-deck:
	CHROME_PATH=$(CHROME_PATH) npx --yes @marp-team/marp-cli@latest $(PITCH_DECK_SRC) --pdf --allow-local-files -o $(PITCH_DECK_SRC:.md=.pdf)
	npx --yes @marp-team/marp-cli@latest $(PITCH_DECK_SRC) --html --allow-local-files -o $(PITCH_DECK_SRC:.md=.html)

CARIDON_DECK_SRC = artifacts/caridon-presentation/slides.md
CARIDON_DECK_PDF = artifacts/caridon-presentation/caridon-presentation.pdf

# Render the Caridon interview presentation to PDF via Marp CLI
caridon-deck:
	CHROME_PATH=$(CHROME_PATH) npx --yes @marp-team/marp-cli@latest $(CARIDON_DECK_SRC) --pdf --allow-local-files -o $(CARIDON_DECK_PDF)

.PHONY: pitch-deck caridon-deck
