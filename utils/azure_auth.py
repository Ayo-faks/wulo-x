...
def should_use_managed_identity_for_acs() -> bool:
    """Resolve ACS authentication overrides before Azure-host auto-detection."""
    ...
    auth_mode = os.getenv("ACS_AUTH_MODE", "").strip().lower()
    if auth_mode in {"entra", "entra_id", "aad", "managed_identity", "default_credential"}:
        return True
    if auth_mode in {"connection_string", "connection-string", "key", "access_key"}:
        return False
