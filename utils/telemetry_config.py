...
def _build_resource_attributes() -> dict[str, str]:
    """Build bounded, non-patient resource identity for every exported signal."""
    service_name = (
    ...
    attributes = {
        "service.name": service_name[:128],
        "service.namespace": os.getenv("SERVICE_NAMESPACE", "callcenter-app")[:128],
        "service.instance.id": _get_instance_id()[:128],
    }
    optional_attributes = {
        "service.environment": os.getenv("ENVIRONMENT")
        or os.getenv("AZURE_APPCONFIG_LABEL")
        or os.getenv("AZURE_ENV_NAME"),
        "service.version": os.getenv("SERVICE_VERSION")
        or os.getenv("APP_VERSION")
        or os.getenv("CONTAINER_APP_REVISION"),
        "deployment.id": os.getenv("DEPLOYMENT_ID")
        or os.getenv("GIT_SHA")
        or os.getenv("CONTAINER_APP_REVISION"),
        "container.image.tag": os.getenv("IMAGE_TAG"),
        "container.app.revision": os.getenv("CONTAINER_APP_REVISION")
        or os.getenv("CONTAINER_APP_REVISION_NAME"),
        "config.hash": os.getenv("CONFIG_HASH"),
        "programme.uuid": os.getenv("PROGRAMME_UUID"),
        "experiment.arm": os.getenv("EXPERIMENT_ARM"),
    }
    attributes.update(
        {
