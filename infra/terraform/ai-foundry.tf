...
resource "azurerm_role_assignment" "ai_foundry_openai_user_for_backend_container" {
  scope                = module.ai_foundry.account_id
...
resource "azurerm_role_assignment" "ai_foundry_openai_user_for_deployment_principal" {
  scope                = module.ai_foundry.account_id
  role_definition_name = "Cognitive Services OpenAI User"
  principal_id         = local.principal_id
}
