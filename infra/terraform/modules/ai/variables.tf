...
variable "foundry_account_name" {
  description = "Name for the Azure AI Foundry account (3-63 lowercase alphanumeric characters; also used as the custom subdomain, whose Azure limit is 63)."
  type        = string
  validation {
    condition     = length(trimspace(var.foundry_account_name)) >= 3 && length(trimspace(var.foundry_account_name)) <= 63
    error_message = "Foundry account name must be between 3 and 63 characters."
  }
}
